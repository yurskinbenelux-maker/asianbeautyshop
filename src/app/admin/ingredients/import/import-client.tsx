// ─────────────────────────────────────────────────────────────────────────
// Client UI for /admin/ingredients/import.
//
// Three-stage flow, no page reloads — same shape as the product import:
//
//   1. Upload — drag-drop or file picker. We read the File → text on the
//      client and ship the string to the server (avoids multipart upload
//      quirks).
//
...
//      re-parses (never trusting client-mutated rows) and writes. Final
...
export function IngredientImportClient() {
  ...
  const loadFile = async (file: File) => {
    ...
    try {
      ...
      console.error("[ingredient-import] file read failed", err);
      setReadError(
  ...
  if (commitState?.ok) {
    return (
      <CommitResultView result={commitState} onStartOver={resetAll} />
    );
  }
    ...
    <div className="space-y-10">
      ...
      {!hasPreview && (
        <form action={previewAction} className="space-y-4">
          ...
          {csvText && <input type="hidden" name="csv" value={csvText} />}
  ...
  return (
    <>
      ...
      >
        ...
        <Td className="font-mono text-[12px] text-ink">{row.slug || "—"}</Td>
        <Td>
          <div className="text-ink">
            {row.displayNameEn || row.inciName || "—"}
          </div>
          {row.displayNameEn && row.inciName && (
      ...
      {expanded && hasDetail && (
        <tr className="border-b border-ink/5 last:border-0 bg-rice/50">
          <td colSpan={4} className="px-4 py-3 text-[12px] text-ink-mid">
            {kind === "error" && (
              <div className="flex items-start gap-2">
                <XCircle
            ...
            {row.warnings.length > 0 && (
              ...
              >
                <AlertTriangle
                  className="mt-0.5 h-3.5 w-3.5 flex-shrink-0 text-ink-mid"
                  aria-hidden
                />
                <ul className="space-y-0.5">
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <p className="mt-2 text-[13px] text-ink-mid">
            Library caches have been refreshed. The new descriptions are
            already live on /ingredients and on every product detail
      ...
      {result.failed.length > 0 && (
        <div className="mt-6 border-t border-ink/10 pt-6">
          ...
          <ul className="mt-3 space-y-1.5 text-[13px] text-ink-mid">
            {result.failed.map((f) => (
              <li key={`${f.rowNumber}-${f.slug}`}>
                <span className="font-mono text-[12px] text-ink">
                  {f.slug || `row ${f.rowNumber}`}
                </span>
                : {f.message}
              </li>
            ))}
